...
impl PlayingStack {
    pub fn empty() -> Self {
        PlayingStack { played_cards_count: 0 }
    }
...
pub struct PlayingStacks {
    cards: CopiesCountingCardCollection,
...
impl PlayingStacks {
    pub fn empty() -> PlayingStacks {
        PlayingStacks {
            cards: CopiesCountingCardCollection::empty(),
            stacks: [
    ...
    pub fn add_card(&mut self, card_id: VariantCardId, variant: &Variant) -> u8 {
        ...
        self.cards.add_card_with_id(card_id);
    ...
    pub fn next_cards(&self, variant: &Variant) -> VariantCardsBitField {
        ...
        for (index, stack) in self.stacks.iter().enumerate().take(variant.number_of_suits as usize) {
...
mod tests {
    ...
    fn should_add_a_card_to_the_correct_stack() {
        ...
        assert!(stacks.cards.contains_card_with_id(6));
    }
