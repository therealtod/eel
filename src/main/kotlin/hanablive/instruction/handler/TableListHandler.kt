...
class TableListHandler(nextHandler: HanabLiveInstructionHandler) : BaseHanabLiveInstructionHandler(nextHandler) {
    ...
    override suspend fun doHandle(messagePayload: String, bot: DefaultHanabLiveBot) {
        val tables: List<hanablive.entity.dto.Table> = mapper.readValue(messagePayload)
        bot.state.setTables(tables)
    }
}
